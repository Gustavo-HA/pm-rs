...
En este capítulo se presentan los fundamentos teóricos que sustentan el desarrollo de la metodología en este trabajo. Se abordan cinco ejes principales: los sistemas de recomendación junto con sus métricas de evaluación (Sección~\ref{sec:recsys}); los fundamentos de procesamiento de lenguaje natural sobre los que se construye la extracción de aspectos y sentimientos a partir de las reseñas (Sección~\ref{sec:nlp}); los modelos de recomendación que incorporan la información extraída por ABSA como señal explícita (Sección~\ref{sec:absa_rs}); las técnicas de optimización metaheurística empleadas para la fusión de componentes (Sección~\ref{sec:metaheuristic}); y los fundamentos de explicabilidad en sistemas de recomendación (Sección~\ref{sec:xai}).
...
Para el español existen varios modelos base ampliamente utilizados --- BETO, MarIA, RoBERTuito --- que difieren en el corpus de preentrenamiento (Wikipedia, BNE, Twitter) y, consecuentemente, en el dominio en el que mejor generalizan \citep{do_deep_2019}. El modelo seleccionado en este trabajo, \textsc{vg055/roberta-base-bne-finetuned-Trip\-Advisor\-Domain\-Adaptation}, parte de un RoBERTa-base preentrenado sobre la Biblioteca Nacional de España (BNE) y posteriormente adaptado al dominio TripAdvisor mediante \textit{fine-tuning} para clasificación de sentimiento de granularidad fina, produciendo cinco clases ordinales:
\begin{equation*}
...
%% ============================================================
%% 2.3  SISTEMAS DE RECOMENDACIÓN CON ABSA
%% ============================================================
\section{Sistemas de recomendación con ABSA} \label{sec:absa_rs}

Una vez que el \textit{pipeline} de NLP descrito en la Sección~\ref{sec:nlp} produce, para cada par (usuario, ítem) y reseña, un conjunto de pares (aspecto, sentimiento), se vuelve posible enriquecer las arquitecturas clásicas de SR con esta información de granularidad fina. La literatura ha explorado dos vías complementarias para integrar las salidas de ABSA en el recomendador: (i) tratar las polaridades aspecto-específicas como \textit{ratings multidimensionales} dentro de un esquema de filtrado colaborativo multicriterio, y (ii) construir representaciones explícitas de \textit{usuario} y de \textit{ítem} en el espacio de aspectos que se combinan con factores latentes en una factorización híbrida. Las dos referencias canónicas que materializan estas vías --- \citet{musto_multi-criteria_2017} y \citet{zhang_explicit_2014} --- constituyen los pilares teóricos de la propuesta híbrida desarrollada en el capítulo de metodología.

\subsection{Filtrado colaborativo multicriterio (Musto et al., 2017)} \label{subsec:musto}
...
\subsection{Modelo de Factores Explícitos --- EFM (Zhang et al., 2014)} \label{subsec:efm}
...
%% ============================================================
%% 2.5  EXPLICABILIDAD EN SISTEMAS DE RECOMENDACIÓN
%% ============================================================
