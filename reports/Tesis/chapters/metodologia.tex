...
estudio de ablación (Sección~\ref{sec:matrices}). A partir de esas matrices se definen los
tres modelos individuales y el protocolo con que se evalúan (Sección~\ref{sec:modelos}).
Por último, los dos modelos basados en aspectos se combinan en un híbrido de un solo
parámetro y se formula el criterio con que se selecciona su peso de fusión
(Sección~\ref{sec:hibrido}). Los resultados de la evaluación, el barrido del peso de fusión
y el análisis del compromiso entre relevancia y cobertura que da título a este trabajo se
...
evaluadas con el modelo de contenido a $K=20$.
...
\begin{table}[ht]
	...
	\caption{Ablación de la construcción de $\mathbf{X}$ e $\mathbf{Y}$. La columna de saturación
		...
		Resultados del modelo de contenido a $K=20$.}
	\label{tab:ablation}
	\begin{tabular}{clccc}
		\toprule
		\textbf{Variante} & \textbf{Construcción de $\mathbf{Y}$}         & \textbf{$\mathbf{Y}$ saturada} & \textbf{NDCG@20} & \textbf{Cov@20} \\
		\midrule
...
Los tres modelos se ajustan sobre las mismas matrices y particiones, y su rendimiento
comparado, junto con las pruebas de significancia que lo sustentan, se reporta en la
Sección~\ref{sec:res_modelos}.

% ============================================================
\section{Modelo híbrido y compromiso relevancia--cobertura}\label{sec:hibrido}

Como se documenta en el Capítulo~\ref{chap:resultados}, la relevancia y la cobertura provienen
de componentes distintos: el modelo de contenido aporta el ordenamiento certero y el
colaborativo la exposición del catálogo. El híbrido las combina mediante la fusión convexa de la
...
colaborativo, de modo que la evaluación de los modelos individuales contiene ya los dos
vértices de la fusión.
...
$\alpha^{\star}(\beta)$ directamente del barrido como indica la
Ecuación~(\ref{eq:alpha_star}). El barrido completo del peso de fusión, el análisis de
sensibilidad del óptimo y el frente de Pareto realizado que ambos inducen se presentan en la
