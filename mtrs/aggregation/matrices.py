...
def compute_user_aspect_importance(df: pd.DataFrame) -> pd.DataFrame:
...
def compute_pueblo_aspect_quality(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Y_{p,j} = 1 + (N-1) / (1 + exp(-h_{p,j} * phi_{p,j}))
