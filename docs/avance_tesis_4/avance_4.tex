...
\begin{frame}{Motivaci\'on del estudio de ablaci\'on}
	...
	\textbf{Dise\~no:} cuatro variantes cruzadas sobre los modelos que dependen de $X$ y/o $Y$ ({\tt CBAttention}, {\tt CBQuality}).
	...
	\begin{columns}[T]
		...
		\begin{column}{0.46\linewidth}
			...
			\begin{itemize}\setlength\itemsep{2pt}
				...
				\item $\log(1+n)$ (A) sub-explota la se\~nal: $\overline{Y}=3.90$.
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{CBAttention \;|\; insensibilidad a $Y$}
	\begin{columns}[c]
		...
		\begin{column}{0.45\linewidth}
			\small
			\textbf{Sanity check:} {\tt CBAttention} s\'olo usa $X$.
...
\begin{frame}{Replanteamiento del h\'ibrido}
	...
	\begin{itemize}\setlength\itemsep{2pt}
		...
		\item {\tt CBAttention} domina la \textbf{cobertura} del cat\'alogo.
		\item Los filtros colaborativos ({\tt CFClassic}, {\tt CFMultiCriteria}) aportan poco frente al de contenido.
	\end{itemize}
...
\begin{frame}{Formulaci\'on de la b\'usqueda en $\alpha$}
	...
	\[
		\hat y_{u,p}(\alpha) \;=\; \alpha\,\hat y^{\mathtt{CBAttention}}_{u,p} \;+\; (1-\alpha)\,\hat y^{\mathtt{CBQuality}}_{u,p},
		\qquad \alpha \in [0,1].
	\]
	...
	\textbf{Implementaci\'on (\texttt{scripts/run\_hybrid\_alpha.py}):}
	\begin{itemize}\setlength\itemsep{2pt}
		\item Una sola pasada: se ajustan {\tt CBAttention} y {\tt CBQuality} y se precomputa el \texttt{score\_stack}.
		\item Por cada $\alpha$ de la grilla ($\alpha\!\in\!\{0, 0.05, \dots, 1\}$ por defecto), s\'olo se re-pesa el stack v\'ia \texttt{set\_weights}.
		\item Logging en MLflow: cada $\alpha$ es un \texttt{step}, las m\'etricas (\texttt{ndcg\_at\_K}, \texttt{coverage\_at\_K}, \dots) generan curvas con $\alpha$ en el eje $x$.
		\item Persistencia v\'ia \texttt{joblib} del h\'ibrido ajustado y de los modelos base $\to$ inferencia reproducible.
	\end{itemize}
\end{frame}
...
\begin{frame}{Pr\'oximos pasos}
	...
	\begin{enumerate}\setlength\itemsep{4pt}
		...
		\item Estudiar \textbf{explicabilidad}: dado $\alpha^{*}$, descomponer la recomendaci\'on en aportaci\'on por aspecto v\'ia $X,Y$.
	...
	\textit{Resta:} evaluaci\'on final del SRH (Obj.\ 6) tras fijar $\alpha^{*}$.
\end{frame}
