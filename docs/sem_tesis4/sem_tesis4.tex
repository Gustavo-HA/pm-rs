	...
	\begin{columns}[T]
		...
		\begin{column}{0.5\linewidth}
			\begin{enumerate}\setlength\itemsep{3pt}
				...
				\item[\color{accentC}\ding{51}] Implementar la fusi\'on de componentes.
				\item[$\square$] Evaluación del SRH y análisis de resultados.
			\end{enumerate}
		\end{column}
	\end{columns}
...
\begin{frame}{Ablaci\'on de $X$ y $Y$\;|\;dise\~no del estudio}
	...
	\vspace{0.5em}
	\textbf{Prueba de cordura.}\;\texttt{CFAttention} s\'olo usa $X$: variantes con
...
\begin{frame}{Resultados\;|\;c\'omo cambian las recomendaciones con $\alpha$}
	...
	\textit{Pueblos en test:} \{Isla Mujeres, Mazamitla, Teotihuacan, Tepoztlan, Tulum\}
	...
	\begin{itemize}\setlength\itemsep{0pt}
		\item $\alpha\!=\!0$: dominan destinos de playa caribe\~na (conjunto que el usuario ya frecuenta).
...
\begin{frame}{Cierre y pr\'oximos pasos}
	...
	\begin{itemize}\setlength\itemsep{2pt}
		...
		\item Reformulaci\'on del h\'ibrido de 4 a 2 modelos (ambos basados en ABSA), grid-search 1D en $\alpha$.
	...
	\begin{enumerate}\setlength\itemsep{2pt}
		\item Exploración sobre las soluciones $\alpha^{*}$ dadas por el parámetro $\beta$ para la obtención de modelos diversos.
		\item Cerrar redacci\'on del Cap\'itulo 4 (resultados) e iniciar correcciones.
	\end{enumerate}
	...
	\textit{Resta:} Evaluación de soluciones $\alpha^{*}$ y análisis de resultados.
\end{frame}
...
\begin{frame}[plain]
	\vfill
	\begin{center}
		{\fontsize{40}{48}\selectfont\bfseries\rmfamily\color{navy}ANEXOS}
	\end{center}
	\vfill
\end{frame}

% ─────────── Anexo: verificacion del sanity check (CFAttention) ────────── %
\begin{frame}{Anexo\;|\;Verificaci\'on del sanity check (\texttt{CFAttention})}
	\begin{columns}[c]
		\begin{column}{0.55\linewidth}
			\centering
		...
		\begin{column}{0.45\linewidth}
			\small
			\textbf{\texttt{CFAttention} es insensible a $Y$.} Variantes con la
			misma $X$ producen NDCG@$K$ id\'enticos en todo el barrido:
			\[
				\text{A}\!=\!\text{D},\qquad \text{B}\!=\!\text{C}.
			\]
			\vspace{0.2em}
			\begin{itemize}\setlength\itemsep{2pt}
				\item Confirma ausencia de fuga entre las ramas $X$ e $Y$ del
				      \emph{pipeline}.
				\item Las diferencias observadas en \texttt{CBQuality} provienen
				      de $Y$, no de ruido ni de desempates aleatorios.
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
