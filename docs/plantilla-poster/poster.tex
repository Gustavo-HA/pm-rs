...
\begin{poster}{
	...
	\begin{tabular}{@{}c l l@{}}
		\toprule
		\textbf{Mat.} & \textbf{Forma}             & \textbf{Significado}        \\
		\midrule
	...
	\headerbox{8.\ An\'alisis de sensibilidad}{name=sensibilidad,column=1,below=modelo,span=1}{
		\centering
		\includegraphics[width=\linewidth]{alpha_sweep.pdf}
		\vskip0.3ex
		\raggedright\footnotesize
		Barrido de $\beta$ (peso de NDCG); cada punto marca el \'optimo $\alpha^{*}$ de su curva:
		% \begin{compactitem}\setlength\itemsep{0pt}
		% 	\item $\beta\!\to\!0$ (prioridad \textbf{cobertura}): $\alpha^{*}$ alto ($\approx 0.8$); domina el modelo colaborativo.
		% 	\item $\beta\!\to\!1$ (prioridad \textbf{relevancia}): $\alpha^{*}\!\to\!0$; domina el modelo basado en contenido.
		% 	\item No hay un $\alpha^{*}$ \'unico: se elige seg\'un el compromiso relevancia--cobertura deseado.
		% \end{compactitem}
	}
	...
	\headerbox{9.\ Conclusiones}{name=conclusiones,column=1,below=sensibilidad,span=1}{
		...
		\begin{compactitem}\setlength\itemsep{0pt}
			...
			\item El h\'ibrido permite obtener modelos diversos que se ajustan a las necesidades del cliente; cobertura del catálogo o relevancia.
		\end{compactitem}
		\vspace{0.3cm}
	}
